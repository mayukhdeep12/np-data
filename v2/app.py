...
if prompt := st.chat_input("Ask about your accounts, cards, transactions…"):
    ...
    with st.chat_message("assistant"):
        ...
        async def run_stream():
            try:
                async for chunk, metadata in _agent.astream(
                    ...
                    # Only stream tokens from the main agent reasoning node.
                    # Guardrail nodes (input_guardrail, output_guardrail) run LLM
                    # classifiers that emit JSON verdict strings — these must never
                    # reach the UI. When a guardrail *blocks* a request it writes a
                    # complete AIMessage into state; that message is surfaced via the
                    # final state read below, not via the stream.
                    if node != "agent":
                        continue
        ...
        while True:
            ...
            if kind == "done":
                # If the agent node never emitted any text, a guardrail blocked
                # the request. The block message lives in the graph state as the
                # last AIMessage — read it synchronously from the checkpointer.
                if not full_response:
                    try:
                        snap = _agent.get_state(config)
